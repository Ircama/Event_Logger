...
#ifndef STANDARD_DF
#include "../fb.h"
extern int fb_x, fb_y, fb_col;

int df_main(void);

#ifdef ARM
#define printf fbprintf
#define bb_printf fbprintf
#define bb_error_msg fbprintf
#define bb_perror_msg fbprintf
#else
...
int df_main()
{
	...
	bb_printf("Filesystem%12s%-15s Used Available Use%% Mounted on\n",
			  "", disp_units_hdr);
	...
	fb_rect(0, 0, 319, 239, fb_colour(0xff, 0xff, 0xff));
	fb_x=0;
	fb_y=0;
	fb_col=fb_colour(0xFF, 0x00, 0x00);

	bb_printf("Filesystem%1s%-12s Used Avail. Use%% Mounted on\n",
			  "", disp_units_hdr);
